// GÜNCELLENDİ: Hatalı kelimeler düzeltildi/kaldırıldı
        ...
        function nextQuestion() { state.currentRound++; console.log(`Next Round: ${state.currentRound + 1} / ${gameSettings.words}`); if (state.currentRound >= gameSettings.words) { console.log("Game Over - All questions presented."); endRound('completed'); return; } if (gameMode === '1p') { const q = state.playerQuestions['1']?.[state.currentRound]; if (q) displayQuestion('1', q); else { console.error(`1P Q ${state.currentRound} not loaded!`); endRound('error'); } } else { state.playerResponses = { '1': null, '2': null }; state.responseTimes = { '1': null, '2': null }; const q1 = state.playerQuestions['1']?.[state.currentRound]; const q2 = state.playerQuestions['2']?.[state.currentRound]; if (q1 && q2) { displayQuestion('1', q1); displayQuestion('2', q2); } else { console.error(`2P Q load error! P1:${!!q1}, P2:${!!q2} for R${state.currentRound}`); endRound('error'); } } if (gameMode === '1p' && p1TimerValueEl) p1TimerValueEl.style.color = (totalTimeLeft <= 10 && totalTimeLeft > 0) ? '#dc3545' : ''; }
        ...
        function start1PGame(difficulty) { console.log("start1PGame:", difficulty); let timerDuration = {'easy': 6, 'medium': 4.5, 'hard': 3}[difficulty] || 5; gameSettings.difficulty = difficulty; gameSettings.timerDuration = timerDuration; document.body.classList.remove('mode-2p'); document.body.classList.add('mode-1p'); difficultyScreen.classList.add('hidden'); gameWrapper.classList.remove('hidden'); setupNewGame(); }
        ...
        function returnToDifficultyScreen() { console.log("returnToDifficultyScreen"); stopGameTimer(); state = {}; modal.style.display = 'none'; gameWrapper.classList.add('hidden'); difficultyScreen.classList.remove('hidden'); rulesScreen.classList.add('hidden'); if (players['1'].wordEl) players['1'].wordEl.textContent = "..."; }
