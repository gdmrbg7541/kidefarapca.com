/* 9. sınıf — 9_4_2 (Günlük Rutin | النِّظام اليَوْمِيّ) */
/* Cümle & diyalog: komisyon Arapça cümleleri temel alınarak Türkçe okuma sırasına göre hizalandı. */
window.data = {
    sentence: [
        {
            words: [
                { tr: "Her gün", order: 3, ar: "كُلَّ يَوْمٍ." },
                { tr: "erkenden", order: 2, ar: "مُبَكِّرًا" },
                { tr: "uyanırım.", order: 1, ar: "أَسْتَيْقِظُ" },
            ]
        },
        {
            words: [
                { tr: "Babamla", order: 3, ar: "مَعَ أَبِي." },
                { tr: "sabah namazını", order: 2, ar: "صَلَاةَ الفَجْرِ" },
                { tr: "kılarım.", order: 1, ar: "أُصَلِّي" },
            ]
        },
        {
            words: [
                { tr: "Saat yedide", order: 3, ar: "فِي السَّاعَةِ السَّابِعَةِ." },
                { tr: "okula", order: 2, ar: "إِلَى المَدْرَسَةِ" },
                { tr: "giderim.", order: 1, ar: "أَذْهَبُ" },
            ]
        },
        {
            words: [
                { tr: "Sınıfta", order: 2, ar: "فِي الفَصْلِ" },
                { tr: "arkadaşlarımla", order: 3, ar: "مَعَ أَصْدِقَائِي." },
                { tr: "ders çalışırım.", order: 1, ar: "أَدْرُسُ" },
            ]
        },
        {
            words: [
                { tr: "Öğlen", order: 3, ar: "فِي الظُّهْرِ." },
                { tr: "ailemle", order: 2, ar: "مَعَ الأُسْرَةِ" },
                { tr: "öğle yemeği yerim.", order: 1, ar: "أَتَغَدَّى" },
            ]
        },
        {
            words: [
                { tr: "Derslerimi", order: 2, ar: "دُرُوسِي" },
                { tr: "akşamdan sonra", order: 3, ar: "بَعْدَ المَسَاءِ." },
                { tr: "okurum.", order: 1, ar: "أَقْرَأُ" },
            ]
        },
        {
            words: [
                { tr: "Akşam", order: 3, ar: "فِي المَسَاءِ." },
                { tr: "futbol", order: 2, ar: "كُرَةَ القَدَمِ" },
                { tr: "oynarım.", order: 1, ar: "أَلْعَبُ" },
            ]
        },
        {
            words: [
                { tr: "Mutfakta", order: 3, ar: "فِي المَطْبَخِ." },
                { tr: "anneme", order: 2, ar: "أُمِّي" },
                { tr: "yardım ederim.", order: 1, ar: "أُسَاعِدُ" },
            ]
        },
        {
            words: [
                { tr: "Her cuma günü", order: 3, ar: "كُلَّ يَوْمِ جُمُعَةٍ." },
                { tr: "odamı", order: 2, ar: "غُرْفَتِي" },
                { tr: "temizlerim.", order: 1, ar: "أُنَظِّفُ" },
            ]
        },
        {
            words: [
                { tr: "Uyumadan önce", order: 3, ar: "قَبْلَ النَّوْمِ." },
                { tr: "derslerimi", order: 2, ar: "دُرُوسِي" },
                { tr: "tekrar ederim.", order: 1, ar: "أُرَاجِعُ" },
            ]
        },
        {
            words: [
                { tr: "Kardeşimle", order: 3, ar: "مَعَ أَخِي." },
                { tr: "televizyon", order: 2, ar: "التِّلْفَازَ" },
                { tr: "izlerim.", order: 1, ar: "أَشَاهِدُ" },
            ]
        },
        {
            words: [
                { tr: "Akşam", order: 3, ar: "فِي المَسَاءِ." },
                { tr: "dedemi", order: 2, ar: "جَدِّي" },
                { tr: "ziyaret ederim.", order: 1, ar: "أَزُورُ" },
            ]
        },
        {
            words: [
                { tr: "Namaz için", order: 3, ar: "لِلصَّلَاةِ." },
                { tr: "camiye", order: 2, ar: "إِلَى المَسْجِدِ" },
                { tr: "giderim.", order: 1, ar: "أَذْهَبُ" },
            ]
        },
        {
            words: [
                { tr: "Bazen", order: 3, ar: "أَحْيَانًا." },
                { tr: "geç", order: 2, ar: "مُتَأَخِّرًا" },
                { tr: "uyurum.", order: 1, ar: "أَنَامُ" },
            ]
        },
        {
            words: [
                { tr: "Günüm", order: 1, ar: "يَوْمِي" },
                { tr: "uzundur", order: 2, ar: "طَوِيلٌ" },
                { tr: "ama", order: 3, ar: "وَلَكِنَّهُ" },
                { tr: "güzeldir.", order: 4, ar: "جَمِيلٌ." },
            ]
        }
    ],
    dialog: [
        {
            p1: [
                { tr: "Her gün", order: 3, ar: "كُلَّ يَوْمٍ؟" },
                { tr: "ne zaman", order: 1, ar: "مَتَى" },
                { tr: "uyanırsın?", order: 2, ar: "تَسْتَيْقِظُ" },
            ],
            p2: [
                { tr: "Her gün", order: 3, ar: "كُلَّ يَوْمٍ." },
                { tr: "erkenden", order: 2, ar: "مُبَكِّرًا" },
                { tr: "uyanırım.", order: 1, ar: "أَسْتَيْقِظُ" },
            ]
        },
        {
            p1: [
                { tr: "Sabah namazını", order: 3, ar: "صَلَاةَ الفَجْرِ؟" },
                { tr: "kiminle", order: 1, ar: "مَعَ مَنْ" },
                { tr: "kılarsın?", order: 2, ar: "تُصَلِّي" },
            ],
            p2: [
                { tr: "Sabah namazını", order: 2, ar: "صَلَاةَ الفَجْرِ" },
                { tr: "babamla", order: 3, ar: "مَعَ أَبِي." },
                { tr: "kılarım.", order: 1, ar: "أُصَلِّي" },
            ]
        },
        {
            p1: [
                { tr: "Akşam", order: 3, ar: "فِي المَسَاءِ؟" },
                { tr: "ne", order: 1, ar: "مَاذَا" },
                { tr: "yaparsın?", order: 2, ar: "تَفْعَلُ" },
            ],
            p2: [
                { tr: "Akşam", order: 3, ar: "فِي المَسَاءِ." },
                { tr: "futbol", order: 2, ar: "كُرَةَ القَدَمِ" },
                { tr: "oynarım.", order: 1, ar: "أَلْعَبُ" },
            ]
        },
        {
            p1: [
                { tr: "Uyumadan önce", order: 3, ar: "قَبْلَ النَّوْمِ؟" },
                { tr: "ne", order: 1, ar: "مَاذَا" },
                { tr: "yaparsın?", order: 2, ar: "تَفْعَلُ" },
            ],
            p2: [
                { tr: "Uyumadan önce", order: 3, ar: "قَبْلَ النَّوْمِ." },
                { tr: "derslerimi", order: 2, ar: "دُرُوسِي" },
                { tr: "tekrar ederim.", order: 1, ar: "أُرَاجِعُ" },
            ]
        }
    ]
};

window.data.words = [
    { tr: "Düzen", ar: "نِظام" },
    { tr: "Günlük", ar: "يَوْمِيّ" },
    { tr: "Uyanıyorum", ar: "أَسْتَيْقِظُ" },
    { tr: "Erken", ar: "مُبَكِّرًا" },
    { tr: "Gidiyorum", ar: "أَذْهَبُ" },
    { tr: "Okul", ar: "مَدْرَسَة" },
    { tr: "Kılıyorum", ar: "أُصَلّي" },
    { tr: "Öğle namazı", ar: "صَلاة الظُّهْر" },
    { tr: "Öğle yemeği yiyorum", ar: "أَتَغَدّى" },
    { tr: "Aile", ar: "أُسْرَة" },
    { tr: "Okuyorum", ar: "أَقْرَأُ" },
    { tr: "Dersler", ar: "دُروس" },
    { tr: "Yazıyorum", ar: "أَكْتُبُ" },
    { tr: "Ödev", ar: "واجِب" },
    { tr: "Oynuyorum", ar: "أَلْعَبُ" },
    { tr: "Saha", ar: "ساحَة" },
    { tr: "Televizyon izliyorum", ar: "أُشاهِدُ التِّلْفاز" },
    { tr: "Yardım ediyorum", ar: "أُسَاعِدُ" },
    { tr: "Dedemi ziyaret ediyorum", ar: "أَزورُ جَدّي" },
    { tr: "Camiye gidiyorum", ar: "أَذْهَبُ إِلى المَسْجِد" },
    { tr: "Yatağımı düzeltiyorum", ar: "أُرَتِّبُ سَريري" },
    { tr: "Odamı temizliyorum", ar: "أُنَظِّفُ غُرْفَتي" },
    { tr: "Derslerimi tekrar ediyorum", ar: "أُراجِعُ دُروسي" },
    { tr: "Geç yatıyorum", ar: "أَنامُ مُتَأَخِّرًا" },
    { tr: "Ne?", ar: "ماذا؟" },
    { tr: "Akşam", ar: "المَساء" },
    { tr: "Nasıl?", ar: "كَيْف" },
];
