...
const TAKIM_RENK = [
  { ad:"الفَريق الأَخْضَر",  tr:"Yeşil",   renk:"#16A085" },
  { ad:"الفَريق الأَزْرَق",  tr:"Mavi",    renk:"#42A5F5" },
...
function takimBilgi(i){
  ...
  const ton = (i * 47) % 360;
  return { ad:"الفَريق " + arRakam(i + 1), tr:(i + 1) + ". takım",
           renk:"hsl(" + ton + " 55% 46%)" };
}
...
const BICIM_AD = { test:"اِخْتِيار", surukle:"تَرْتيب", eslestir:"وَصْل", yazma:"كِتابَة",
  bosluk:"مَلْء الفَراغ", dogruyanlis:"صَحّ أَمْ خَطَأ", cumlesira:"تَرْتيب الجُمْلَة" };
...
const D = {
  ...
  anlKaynak: "",                       // "akis": 1. sayfadan açıldı · "bas": Başlat kartından
  karsilama: true,                     // açılışta karşılama ekranı: mod anahtarı + akış
  ...
  mod: "canli",                // canli | cevrimdisi — açılışta canlı, tek kurulum iki çıkış
...
/* ka.com sürümü: liste ve küme adları zaten Türkçe (KONULAR[].ad ve
   UNITELER[].ad). uniteTr/dersTr bu sözlükler boşken doğrudan o adlara
   düşer — komisyon dosyasındaki 7. sınıf ünite adları buraya taşınmaz. */
const UNITE_TR = {};
const DERS_TR = {};
...
/* Karşılama başlığındaki küçük kupa: hafifçe sallanır, yıldızı parıldar. */
CIZ.kupaKucuk = `<svg viewBox="0 0 48 48" class="ak-ik ak-kupa-k" aria-hidden="true">
  <g class="ak-kupa-govde">
    <path d="M15 9 h18 v10 a9 9 0 0 1 -18 0 Z" fill="#F6C445"/>
    <path d="M15 12 h-5 a5 5 0 0 0 5 8 M33 12 h5 a5 5 0 0 1 -5 8" fill="none" stroke="#D69E2E" stroke-width="2.6" stroke-linecap="round"/>
    <rect x="21" y="27" width="6" height="6" fill="#D69E2E"/>
    <rect x="14" y="33" width="20" height="5" rx="2" fill="#B7791F"/>
    <path d="M24 12 l1.5 3 3.3.5 -2.4 2.3 .6 3.3 -3-1.6 -3 1.6 .6-3.3 -2.4-2.3 3.3-.5 Z" fill="#FFF3C4"/>
  </g>
  <path class="ak-yildiz ak-y1" d="M9 6 l1 2 2.2.3 -1.6 1.5 .4 2.2 -2-1 -2 1 .4-2.2 -1.6-1.5 2.2-.3 Z" fill="#FFC94A"/>
  <path class="ak-yildiz ak-y2" d="M40 8 l1 2 2.2.3 -1.6 1.5 .4 2.2 -2-1 -2 1 .4-2.2 -1.6-1.5 2.2-.3 Z" fill="#FFC94A"/>
</svg>`;
...
function switchHtml(aktif, pasif){
  ...
  return `<div class="cdw-switch${pasif?" pasif":""}" role="group" aria-label="Yarışma modu"
       title="${ipucu}">
    ${yol.map(([v, ad, not, ciz]) => `<button type="button" class="cdw-sw${aktif===v?" ac":""}" data-mod="${v}"
...
function eksikler(){
  const e = [];
  if (!D.konuId) e.push({ ad: "ders", yazi: "Ders seçilmedi", git: "ders", hedef: "konu" });
  else if (!seciliSorular().length)
    e.push({ ad: "ders", yazi: "Seçtiğin süzgeçlerde soru kalmadı", git: "ders", hedef: "suzgec" });
  else {
    ...
    if (s < h)
      e.push({ ad: "ders", git: "ders", hedef: "havuz",
               yazi: s ? "Havuzdan " + h + " soru seçmelisin — şu an " + s + " seçili"
                       : "Havuzdan " + h + " soru seç" });
  }
  if (D.mod === "cevrimdisi"){
    if (D.bicim === "sinif" && !sinifAdlari().length)
      e.push({ ad: "bicim", yazi: "Yarışan sınıf yazılmadı", git: "bicim", hedef: "liste" });
    else if (!D.katilim.length)
      e.push({ ad: "bicim", yazi: "Takım/sınıf listesi boş", git: "bicim", hedef: "liste" });
  }
  return e;
}
...
function turImza(){
  ...
  return [D.konuId || "", D.soruSayisi,
          ...
          (typeof state !== "undefined" && state.icerikSecim)
            ? Object.keys(state.icerikSecim).map(k => state.icerikSecim[k] === false ? 0 : 1).join("") : "",
...
/* ---- Adım geçişinde yay efekti ----
   Kırmızı "şimdi" dairesi eski yerinden kopar, esneyerek yeni sekmeye
   kayar ve yaylanarak oturur; eski yerinde bir an bir hayalet küçülür. */
function yolKonumAl(){
  const s = document.querySelector("#ekranCevrimdisi .cdw-yol .simdi .cdw-yol-no, #cdYolCanli .cdw-yol .simdi .cdw-yol-no");
  if (!s || D._yolAdim == null) return null;
  const r = s.getBoundingClientRect();
  return { adim: D._yolAdim, x: r.left, y: r.top, w: r.width, h: r.height };
}
function yolYayla(onceki){
  const yeni = document.querySelector("#ekranCevrimdisi .cdw-yol .simdi .cdw-yol-no, #cdYolCanli .cdw-yol .simdi .cdw-yol-no");
  const suAdim = D.adim;
  D._yolAdim = suAdim;
  if (!yeni || !onceki || onceki.adim === suAdim) return;
  const r = yeni.getBoundingClientRect();
  const dx = onceki.x - r.left;
  if (Math.abs(dx) < 2) return;
  yeni.style.setProperty("--dx", dx + "px");
  yeni.classList.remove("cdw-yol-kayan"); void yeni.offsetWidth;
  yeni.classList.add("cdw-yol-kayan");
  /* eski yerde kopan hayalet */
  const h = document.createElement("span");
  h.className = "cdw-yol-hayalet";
  h.style.cssText = `left:${onceki.x}px;top:${onceki.y}px;width:${onceki.w}px;height:${onceki.h}px;--yon:${dx < 0 ? 1 : -1}`;
  document.body.appendChild(h);
  setTimeout(() => h.remove(), 500);
}

/* Eksik kutuyu kırmızı halkayla vurgula: ders kutusu, soru havuzu kutusu,
   süzgeç kutusu ya da takım listesi. Birkaç saniye sonra söner. */
function eksikVurgula(hedef, yazi){
  const sec = { konu: ".cdw-panel .biy-konu-secim", havuz: ".cdw-panel .biy-soru-sec-secim",
                suzgec: "#cdPanelAyar", liste: ".cdw-liste-bolum" }[hedef];
  if (!sec) return;
  requestAnimationFrame(() => {
    const k = document.querySelector(sec); if (!k) return;
    document.querySelectorAll(".cdw-eksik-vurgu").forEach(x => x.classList.remove("cdw-eksik-vurgu"));
    document.querySelectorAll(".cdw-eksik-rozet").forEach(x => x.remove());
    k.classList.remove("cdw-eksik-vurgu"); void k.offsetWidth;
    k.classList.add("cdw-eksik-vurgu");
    const r = document.createElement("span");
    r.className = "cdw-eksik-rozet"; r.textContent = yazi || "Burası eksik";
    k.appendChild(r);
    try { k.scrollIntoView({ block: "center", behavior: "smooth" }); } catch(e){}
    if (k._eksikZ) clearTimeout(k._eksikZ);
    k._eksikZ = setTimeout(() => { k.classList.remove("cdw-eksik-vurgu"); r.remove(); }, 4200);
  });
}
...
/* ---- Akış adımlarının küçük animasyonlu çizimleri (48×48) ----
   Her biri kendi CSS sınıfıyla oynar (cdw-akis.css bölümü). */
const AKIS_IK = {
  ders: `<svg viewBox="0 0 48 48" class="ak-ik ak-ders" aria-hidden="true">
    <rect x="9" y="7" width="30" height="36" rx="5" fill="#fff" stroke="#C7D3E0" stroke-width="2.2"/>
    <rect x="15" y="3" width="18" height="7" rx="3" fill="#94A6B8"/>
    <path d="M16 20 h16 M16 27 h16 M16 34 h10" stroke="#DCE6F0" stroke-width="2.6" stroke-linecap="round"/>
    <g class="ak-ders-tik"><circle cx="31" cy="33" r="7" fill="#16A085"/>
      <path d="M27.5 33.2 l2.4 2.4 4.4-4.8" fill="none" stroke="#fff" stroke-width="2.2" stroke-linecap="round" stroke-linejoin="round"/></g>
  </svg>`,
  kimler: `<svg viewBox="0 0 48 48" class="ak-ik ak-kimler" aria-hidden="true">
    <g class="ak-k1"><circle cx="13" cy="19" r="5.5" fill="#F7D2B0"/><path d="M4 38 v-8 a9 9 0 0 1 18 0 v8 Z" fill="#F39C12"/></g>
    <g class="ak-k3"><circle cx="35" cy="19" r="5.5" fill="#FADFC4"/><path d="M26 38 v-8 a9 9 0 0 1 18 0 v8 Z" fill="#EF5350"/></g>
    <g class="ak-k2"><circle cx="24" cy="15" r="6.5" fill="#EFC49A"/><path d="M13 40 v-9 a11 11 0 0 1 22 0 v9 Z" fill="#42A5F5"/></g>
  </svg>`,
  karekod: `<svg viewBox="0 0 48 48" class="ak-ik ak-karekod" aria-hidden="true">
    <rect x="7" y="7" width="34" height="34" rx="5" fill="#fff" stroke="#C7D3E0" stroke-width="2.2"/>
    <g fill="#1F3864">
      <path d="M12 12 h9 v9 h-9 Z M15 15 v3 h3 v-3 Z" fill-rule="evenodd"/>
      <path d="M27 12 h9 v9 h-9 Z M30 15 v3 h3 v-3 Z" fill-rule="evenodd"/>
      <path d="M12 27 h9 v9 h-9 Z M15 30 v3 h3 v-3 Z" fill-rule="evenodd"/>
      <rect x="27" y="27" width="3" height="3"/><rect x="33" y="27" width="3" height="3"/>
      <rect x="30" y="30" width="3" height="3"/><rect x="27" y="33" width="3" height="3"/><rect x="33" y="33" width="3" height="3"/>
    </g>
    <rect class="ak-tara" x="9" y="10" width="30" height="2.4" rx="1.2" fill="#EF5350"/>
  </svg>`,
  yarisma: `<svg viewBox="0 0 48 48" class="ak-ik ak-yarisma" aria-hidden="true">
    <rect x="4" y="8" width="30" height="21" rx="3" fill="#1F3864"/>
    <rect x="15" y="29" width="8" height="4" fill="#94A6B8"/><rect x="10" y="33" width="18" height="2.5" rx="1.2" fill="#94A6B8"/>
    <text x="19" y="24" text-anchor="middle" font-size="14" font-weight="900" fill="#FFC94A" font-family="Arial">?</text>
    <g class="ak-tel"><rect x="31" y="18" width="13" height="24" rx="3" fill="#fff" stroke="#C7D3E0" stroke-width="2"/>
      <rect x="33.5" y="22" width="8" height="12" rx="1.5" fill="#F7FAFD"/>
      <circle class="ak-tel-nokta" cx="37.5" cy="28" r="2.6" fill="#16A085"/></g>
  </svg>`,
  sonuc: `<svg viewBox="0 0 48 48" class="ak-ik ak-sonuc" aria-hidden="true">
    <path d="M15 8 h18 v9 a9 9 0 0 1 -18 0 Z" fill="#F6C445"/>
    <path d="M15 11 h-5 a5 5 0 0 0 5 8 M33 11 h5 a5 5 0 0 1 -5 8" fill="none" stroke="#D69E2E" stroke-width="2.6" stroke-linecap="round"/>
    <rect x="21" y="25" width="6" height="7" fill="#D69E2E"/><rect x="15" y="32" width="18" height="5" rx="2" fill="#B7791F"/>
    <path class="ak-yildiz ak-y1" d="M9 24 l1.2 2.4 2.6.4 -1.9 1.8 .5 2.6 -2.4-1.2 -2.4 1.2 .5-2.6 -1.9-1.8 2.6-.4 Z" fill="#FFC94A"/>
    <path class="ak-yildiz ak-y2" d="M40 6 l1 2 2.2.3 -1.6 1.5 .4 2.2 -2-1 -2 1 .4-2.2 -1.6-1.5 2.2-.3 Z" fill="#FFC94A"/>
    <path class="ak-yildiz ak-y3" d="M38 28 l1 2 2.2.3 -1.6 1.5 .4 2.2 -2-1 -2 1 .4-2.2 -1.6-1.5 2.2-.3 Z" fill="#FFC94A"/>
  </svg>`,
  takimcikti: `<svg viewBox="0 0 48 48" class="ak-ik ak-cikti" aria-hidden="true">
    <g class="ak-k1"><circle cx="11" cy="14" r="5" fill="#F7D2B0"/><path d="M3 31 v-7 a8 8 0 0 1 16 0 v7 Z" fill="#F39C12"/></g>
    <g class="ak-k2"><circle cx="24" cy="12" r="5.5" fill="#EFC49A"/><path d="M15 31 v-8 a9 9 0 0 1 18 0 v8 Z" fill="#42A5F5"/></g>
    <rect x="27" y="26" width="18" height="11" rx="2.5" fill="#94A6B8"/>
    <rect class="ak-kagit" x="30" y="16" width="12" height="13" rx="1.5" fill="#fff" stroke="#C7D3E0" stroke-width="1.8"/>
    <path class="ak-kagit-cizgi" d="M33 21 h6 M33 24.5 h4" stroke="#DCE6F0" stroke-width="1.8" stroke-linecap="round"/>
    <rect x="29" y="37" width="14" height="4" rx="1.5" fill="#5A6B7C"/>
  </svg>`,
  tahta: `<svg viewBox="0 0 48 48" class="ak-ik ak-tahta" aria-hidden="true">
    <rect x="5" y="7" width="38" height="25" rx="3" fill="#1F3864"/>
    <rect x="9" y="11" width="30" height="17" rx="2" fill="#2A4A80"/>
    <path d="M14 17 h20 M14 22 h12" stroke="#BFD3F5" stroke-width="2.4" stroke-linecap="round"/>
    <rect x="9" y="29" width="30" height="2" fill="#EF5350" opacity=".35"/>
    <rect class="ak-sure" x="9" y="29" width="30" height="2" fill="#EF5350"/>
    <rect x="20" y="32" width="8" height="4" fill="#94A6B8"/><rect x="14" y="36" width="20" height="3" rx="1.4" fill="#94A6B8"/>
  </svg>`,
  puan: `<svg viewBox="0 0 48 48" class="ak-ik ak-puan" aria-hidden="true">
    <rect x="6" y="9" width="36" height="9" rx="3" fill="#F7FAFD" stroke="#C7D3E0" stroke-width="1.8"/>
    <rect x="6" y="21" width="36" height="9" rx="3" fill="#F7FAFD" stroke="#C7D3E0" stroke-width="1.8"/>
    <rect x="6" y="33" width="36" height="9" rx="3" fill="#F7FAFD" stroke="#C7D3E0" stroke-width="1.8"/>
    <rect x="10" y="12" width="14" height="3" rx="1.5" fill="#DCE6F0"/><rect x="10" y="24" width="14" height="3" rx="1.5" fill="#DCE6F0"/><rect x="10" y="36" width="14" height="3" rx="1.5" fill="#DCE6F0"/>
    <g class="ak-tik ak-t1"><circle cx="35" cy="13.5" r="3.6" fill="#16A085"/><path d="M33 13.6 l1.4 1.4 2.6-2.8" fill="none" stroke="#fff" stroke-width="1.6" stroke-linecap="round" stroke-linejoin="round"/></g>
    <g class="ak-tik ak-t2"><circle cx="35" cy="25.5" r="3.6" fill="#EF5350"/><path d="M33.2 23.7 l3.6 3.6 M36.8 23.7 l-3.6 3.6" stroke="#fff" stroke-width="1.6" stroke-linecap="round"/></g>
    <g class="ak-tik ak-t3"><circle cx="35" cy="37.5" r="3.6" fill="#16A085"/><path d="M33 37.6 l1.4 1.4 2.6-2.8" fill="none" stroke="#fff" stroke-width="1.6" stroke-linecap="round" stroke-linejoin="round"/></g>
  </svg>`,
  siralama: `<svg viewBox="0 0 48 48" class="ak-ik ak-sira" aria-hidden="true">
    <rect class="ak-p2" x="5" y="24" width="12" height="18" rx="2" fill="#C0C8D2"/>
    <rect class="ak-p1" x="18" y="14" width="12" height="28" rx="2" fill="#F6C445"/>
    <rect class="ak-p3" x="31" y="29" width="12" height="13" rx="2" fill="#D69E2E"/>
    <text x="24" y="29" text-anchor="middle" font-size="11" font-weight="900" fill="#7A4E00" font-family="Arial">1</text>
    <path class="ak-yildiz ak-y1" d="M24 3 l1.4 2.8 3.1.5 -2.2 2.2 .5 3.1 -2.8-1.5 -2.8 1.5 .5-3.1 -2.2-2.2 3.1-.5 Z" fill="#FFC94A"/>
  </svg>`
};

/* ---- Uygulama akışı: en başta "ne yapacağım?" bir bakışta ----
   İlk üç adım sekmelerin ta kendisi; sonrası yarışma sırasında olacaklar. */
function akisAdimlari(){
  return akisAdimlariMod(D.mod);
}
function akisAdimlariMod(mod){
  if (mod === "canli") return [
  ...
  return [
    { ik:AKIS_IK.ders,       bas:"Ders ve sorular", alt:"Süzgeç ve havuzdan soru seçimi" },
    { ik:AKIS_IK.takimcikti, bas:kim + " ve çıktı",  alt:"Liste ve isteğe bağlı PDF" },
    { ik:AKIS_IK.tahta,      bas:"Tahtaya yansıt",   alt:"Herkes defterine yazar" },
    { ik:AKIS_IK.puan,       bas:"Cevabı aç, puanla", alt:"Her takıma ✓ ya da ✗ ver" },
    { ik:AKIS_IK.siralama,   bas:"Sıralama",         alt:"Puan durumu ve kupa" }
  ];
}
function akisHtml(buyuk, mod){
  const a = akisAdimlariMod(mod || D.mod);
  return `
  <div class="cdw-akis${buyuk ? " cdw-akis-buyuk" : ""}" role="list" aria-label="Uygulama akışı"
       style="--n:${a.length}">
    ${a.map((x, i) => `<div class="cdw-akis-adim${i < 3 ? " sekme" : ""}" role="listitem" style="--i:${i}">
        <span class="cdw-akis-no">${i + 1}</span>
        <span class="cdw-akis-ik" aria-hidden="true">${x.ik}</span>
        <span class="cdw-akis-yazi"><b>${kacisi(x.bas)}</b><small>${kacisi(x.alt)}</small></span>
      </div>${i < a.length - 1 ? `<span class="cdw-akis-ok" aria-hidden="true" style="--i:${i}">›</span>` : ""}`).join("")}
    ${buyuk ? "" : `<button type="button" class="cdw-akis-tus" onclick="COFF.akisAc()"
            title="Adım adım anlatım">Nasıl işler? ›</button>`}
  </div>`;
}

/* ---- Karşılama ekranı: mod anahtarı + uygulama akışı, kurulumdan önce ---- */
function karsilamaHtml(){
  const canli = D.mod === "canli";
  return `
  <div class="cdw cdw-karsilama" dir="ltr">
    <div class="cdw-ust">
      ...
      <ol class="cdw-yol"><li class="simdi"><button type="button" class="cdw-yol-tus" disabled>
        <span class="cdw-yol-no cdw-yol-kupa">${CIZ.kupaKucuk}</span><span class="cdw-yol-ad">Bilgi Yarışması</span></button></li></ol>
    </div>
    <div class="cdw-govde cdw-govde-karsilama">
      <div class="cdw-sahne cdw-genis cdw-kars-sahne">
        ...
        <section class="cdw-mod-bolum${D.mod === m ? " secili" : ""} cdw-mod-${m}"
                 onclick="COFF.modSec('${m}')" role="button" tabindex="0" aria-pressed="${D.mod === m}"
                 onkeydown="if(event.key==='Enter'||event.key===' '){event.preventDefault();COFF.modSec('${m}');}">
          <div class="cdw-mod-bas">
            <span class="cdw-mod-ciz-yuva">${ciz}</span>
            <div class="cdw-mod-yazi">
              <h2 class="cdw-mod-ad">${ad}<span class="cdw-mod-secili-rozet">Seçili ✓</span></h2>
              <p class="cdw-mod-kisa">${kisa}</p>
              <p class="cdw-mod-uzun">${uzun}</p>
            </div>
            <div class="cdw-mod-tuslar">
              <button type="button" class="cdw-tus" onclick="event.stopPropagation();COFF.modAkis('${m}')">Nasıl işler? ›</button>
              <button type="button" class="cdw-tus cdw-tus-ana" onclick="event.stopPropagation();COFF.modBasla('${m}')">${ad} başla ›</button>
            </div>
          </div>
          ${akisHtml(true, m)}
        </section>`).join("")}
...
function adim5(){
  ...
  return `
  <div class="cdw-sahne">
    <h2 class="cdw-bas">${hazir ? "Hazır! Tahtaya yansıtalım mı?" : "Bir adım kaldı"}</h2>
    ${eks.length ? `
    <div class="cdw-eksik" role="status">
      ...
      <ul class="cdw-eksik-liste">
        ${eks.map(x => `<li><b>${kacisi(x.yazi)}</b>
          <button type="button" class="cdw-eksik-tus" onclick="COFF.eksigeGit(${adimNo(x.git)}, '${x.hedef || ""}', '${kacisi(x.yazi).replace(/'/g, "&#39;")}')">
    ...
    <!-- Turnuva kartı kaldırıldı: çevrimdışında tek yol "Tahtaya yansıt". -->
...
function anlatimPerdeleri(){
  if (D.mod === "canli") return [
    ...
    { bas: "Sonuç ve kupa",
      alt: "Son sorudan sonra sıralama açıklanır, kazanan kutlanır.",
      ciz: CIZ.kupa, sinif: "p5" }
  ];
  ...
  const n = D.sorular.length || hedefSoru();
  return [
    { bas: "Sorular ve takımlar",
      alt: "1. sekmede dersi seç ve havuzdan soruları işaretle. 2. sekmede takım ya da sınıf listesini kur; istersen soru kitapçığını ve yarışma kâğıdını PDF olarak yazdır.",
      ciz: CIZ.liste, sinif: "p0" },
    { bas: "Defter ve kalem hazır olsun",
      alt: "Herkes defterine 1'den " + n + "'e kadar numara yazsın. " + kimNot,
    ...
    { bas: "Cevabı aç, puanı işle",
      ...
      ciz: CIZ.puanSatir, sinif: "p4" },
    { bas: "Sıralama ve kupa",
      alt: "Her sorudan sonra puan durumu görünür; son soruda sıralama açıklanır.",
      ciz: CIZ.kupa, sinif: "p5" }
  ];
}
...
function anlatimHtml(){
  ...
  <div class="cdw" dir="ltr">
    <div class="cdw-ust">
      <button type="button" class="cdw-geri-tus" onclick="${D.anlKaynak === "akis" ? "COFF.anlatimKapat()" : "COFF.basaDon()"}"
      ...
      <button type="button" class="cd-tus cd-tus-ufak" onclick="${D.anlKaynak === "akis" ? "COFF.anlatimKapat()" : "COFF.anlatimBitir()"}">${D.anlKaynak === "akis" ? "Kapat ✕" : "Atla ›"}</button>
    </div>
    ...
    <div class="cdw-alt">
      ...
      ${i < p.length - 1
        ...
        : (D.anlKaynak === "akis"
            ? `<button type="button" class="cdw-tus cdw-tus-ana" onclick="COFF.anlatimKapat()">Anladım, kuruluma dön ›</button>`
...
const yonSade = t => String(t || "").replace(/[\u064B-\u0652\u0670\u0640]/g, "")
                                    .replace(/\s+/g, " ").trim();
/* Fiyat/ağırlık ipuçlu sorularda ürün adları (harekesiz) → Türkçe. */
const URUN_TR = {
  "الطماطم":"Domates", "الموز":"Muz", "التفاح":"Elma", "الكرز":"Kiraz", "المشمش":"Kayısı",
  "البرتقال":"Portakal", "العنب":"Üzüm", "الخيار":"Salatalık", "البطاطا":"Patates",
  "الجزر":"Havuç", "البصل":"Soğan", "البيض":"Yumurta", "الحليب":"Süt", "الجبن":"Peynir",
  "العسل":"Bal", "الباذنجان":"Patlıcan", "الفلفل":"Biber", "الخبز":"Ekmek", "الزيتون":"Zeytin"
};
const urunTr = a => URUN_TR[a] || a;
const YON_TABLO = [
  ...
  [/^(\S+) ب (\d+) ليرات و(\S+) ب (\d+) ليرات\. «[^»]*» — صحيح؟$/,
    m => urunTr(m[1]) + " " + m[2] + " lira, " + urunTr(m[3]).toLowerCase() + " " + m[4] + " lira. Aşağıdaki cümle doğru mu?", 1],
  /* Mantık yürütmeli ism-i tafdil: ipucu Türkçe, tamamlanacak cümle Arapça. */
  [/^(\S+) ب (\d+) ليرات و(\S+) ب (\d+) ليرات\. أكمل: «[^»]*»$/,
    m => urunTr(m[1]) + " " + m[2] + " lira, " + urunTr(m[3]).toLowerCase() + " " + m[4] + " lira. Cümleyi tamamla.", 1],
  [/^(\S+) (\d+) كيلو و(\S+) (\d+) كيلو\. أكمل: «[^»]*»$/,
    m => urunTr(m[1]) + " " + m[2] + " kilo, " + urunTr(m[3]).toLowerCase() + " " + m[4] + " kilo. Cümleyi tamamla.", 1],
  /* Bağlamlı boşluk: parantezdeki Türkçe ipucu boşluğa gelecek kelime. */
  [/^أكمل: «[^»]*» \(([^)]+)\)$/, m => "Boşluğu tamamla: " + m[1], 1],
  [/^ما مثنى «[^»]*»؟$/, "Bu kelimenin ikili (müsennâ) hâli hangisidir?", 1],
  [/^صل المفرد بالمثنى\.$/, "Tekilleri ikili hâlleriyle eşleştir.", 0],
  [/^صل عبارات السوق بمعانيها\.$/, "Alışveriş ifadelerini anlamlarıyla eşleştir.", 0],
...
const COFF = {
  ...
  geriCik(){
    ...
    COFF.karsilamaAc();                  // ilk adımda: karşılama ekranına dön
  },
  ciz(){
    const e = el("ekranCevrimdisi");
    if (!e) return;
    if (D.karsilama){ e.innerHTML = karsilamaHtml(); D._yolAdim = null; return; }
    const onceki = yolKonumAl();
    e.innerHTML = kurulumHtml();
    panelYerlestir();        // taşınan gerçek düğümleri yuvalarına yerleştir
    yolYayla(onceki);
  },
  /* Karşılamadan kuruluma; 1. adımın geri oku karşılamaya döner. */
  basla(){ D.karsilama = false; D.adim = 1; COFF.ciz(); COFF._adim1Vurgu(); },
  /* Karşılamadan kuruluma girince sıra anlatılır: önce süzgeç kutusu,
     sonra ders ve soru havuzu kutusu birer kez parlar. */
  _adim1Vurgu(){
    const a = el("cdPanelAyar"), d = el("cdPanelDers");
    if (!a || !d) return;
    if (COFF._vurguZ) COFF._vurguZ.forEach(clearTimeout);
    [a, d].forEach(x => x.classList.remove("cdw-vurgu"));
    a.dataset.vurgu = "1 · Önce süzgeçler";
    d.dataset.vurgu = "2 · Sonra ders ve soru havuzu";
    a.classList.add("cdw-vurgu");
    COFF._vurguZ = [
      setTimeout(() => { a.classList.remove("cdw-vurgu"); d.classList.add("cdw-vurgu"); }, 2800),
      setTimeout(() => { d.classList.remove("cdw-vurgu"); }, 5600)
    ];
  },
  ...
  karsilamaAc(){ COFF.sunumKapat(); D.karsilama = true; ekranGoster("ekranCevrimdisi"); COFF.ciz(); },
  ...
  /* ---- adım adım gezinme ---- */
  adimGit(n){
    D.karsilama = false;                 // bir adıma gidiliyorsa karşılama bitti
    const yol = adimlar();
    ...
    if (D.mod === "canli" && hedef === "karekod"){
      ...
      const eks = eksikler();
      if (eks.length){ COFF.eksigeGit(adimNo(eks[0].git), eks[0].hedef, eks[0].yazi); return; }
  ...
  /* Eksik olan sekmeye git, eksik kutuyu kırmızıyla vurgula. */
  eksigeGit(n, hedef, yazi){
    COFF.adimGit(n);
    if (yazi) uyar(yazi);
    eksikVurgula(hedef, yazi);
  },
  ...
  adimIleri(){
    const k = adimAnahtar();
    if (k === "ders"){
      const eks = eksikler().find(x => x.ad === "ders");
      if (eks){ uyar(eks.yazi); eksikVurgula(eks.hedef, eks.yazi); return; }
    }
  ...
  defterAc(){
    ...
    D.anlAdim = 0; D.anlKaynak = "bas";
    const e = el("ekranCevrimdisi"); if (e) e.innerHTML = anlatimHtml();
  },
  /* 1. sayfadaki akış şeridinden: soru hazır olmasa da anlatım açılır,
     bitince kuruluma dönülür. */
  akisAc(){
    D.anlAdim = 0; D.anlKaynak = "akis";
    const e = el("ekranCevrimdisi"); if (e) e.innerHTML = anlatimHtml();
  },
  anlatimKapat(){ D.anlKaynak = ""; D.anlatimGoruldu = true; COFF.ciz(); },
...
(function(){
  ...
  const kurulumGerekli = () => {
    ...
    /* --- ka.com uyarlaması ---
       Sitede iki ekran daha var: rol/giriş kapısı ve MOD KAPISI. Emniyet
       kontrolü bunları da atlamalı; yoksa öğretmen "canlı mı çevrimdışı mı"
       diye sorulan ekranda dururken kurulum kendiliğinden açılıyor ve
       kapı işlevsiz kalıyordu. Öğretmen seçimini yapana kadar bekle. */
    if (gorunur("ekranMod") || gorunur("ekranGirisKapisi")) return false;
    if (gorunur("ekranAnasayfa")) return false;      // canlı kurulum açık
    return true;
  };
  const emniyet = () => { if (kurulumGerekli()) COFF.ac(D.mod); };

  const yaz = () => {
    /* ka.com uyarlaması: açılışta KENDİLİĞİNDEN açılmaz. "ekranYukleniyor"
       burada rol kontrolü ekranı; mod kapısı da öğretmenin seçimini bekler.
       Çevrimdışı kurulum yalnız BIY.modSec('cevrimdisi') ile açılır. */
