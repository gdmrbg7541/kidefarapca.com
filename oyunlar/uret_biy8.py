...
# -*- coding: utf-8 -*-
"""
BİLGİ YARIŞMASI — 8. SINIF KONUSUNU ÜRETİR
==========================================
Kaynak : muhadese/veri/8_1_1 … 8_6_3 (18 ders, 6 ünite) kelime ve cümleleri
Çıktı  : oyunlar/bilgiyarismasikacom.js içindeki  { id: "sinif8", … }  bloğu

uret_biy6.py'nin 8. sınıfa uyarlanmışı; soru türleri, sayıları ve id
aralıkları BİREBİR aynı (yarışma ekranı sınıflar arasında aynı densin):
    1 …  8999  kelime soruları
    9000 … 9999  kelime oyunları (eşleştir / doğru-yanlış / sırala / yaz)
   20000 … 20999 cümle soruları

İKİ FARK:
  1) Tohumlar 6xxx yerine 8xxx — aynı kelime iki sınıfta da geçse soru
     dizilişi aynı çıkmasın.
  2) Ders verisi node ile okunuyor. 8. sınıf dosyaları JSON değil, JS
     nesne değişmezi (tırnaksız anahtar, sondaki virgül); json.loads
     onları okuyamıyor — 6. sınıfın betiği burada patlıyordu.

Betik YİNELENEBİLİR: dosyada zaten bir sinif8 bloğu varsa onu değiştirir.
Ders verisi büyürse yeniden çalıştırılır ve sistem/sinifveri.js'teki
`sinif8.soru` sayısı da güncellenir.
"""
import json, re, io, os, subprocess, sys

KOK = os.path.dirname(os.path.dirname(os.path.abspath(__file__)))
DERSLER = ['8_1_1', '8_1_2', '8_1_3', '8_2_1', '8_2_2', '8_2_3',
           '8_3_1', '8_3_2', '8_3_3', '8_4_1', '8_4_2', '8_4_3',
           '8_5_1', '8_5_2', '8_5_3', '8_6_1', '8_6_2', '8_6_3']
...
# Ders dosyaları JS değişmezi; node ile okunup JSON'a çevriliyor.
NODE = r'''
/* node -e ile cagrilinca argv[0]=node, argv[1] ILK kullanici argumani
   olur ("--" yutulur). Once slice(3) yazip yanlis yoldan okumustuk. */
const fs = require('fs'), vm = require('vm');
const arg = process.argv.slice(1).filter(function (a) { return a !== '--'; });
const kok = arg[0], dersler = arg.slice(1);
const cikti = {};
for (const d of dersler) {
  const ctx = { window: {}, console };
  vm.createContext(ctx);
  vm.runInContext(fs.readFileSync(kok + '/muhadese/veri/' + d + '.js', 'utf8'), ctx);
  cikti[d] = ctx.window.data || null;
}
...
def dersVerisi():
    p = subprocess.run(['node', '-e', NODE, KOK] + DERSLER,
...
def uret():
    ...
    kid = 1
    for i, k in enumerate(karisik(kelimeler, 8001)):
        if kid > 60:
            break
        sec, dg = secenekKur(k['tr'], celdirici(k['tr'], trHavuz, 8700 + i), 8750 + i)
    ...
    for i, k in enumerate(karisik(kelimeler, 8002)):
        if kid > 80:
            break
        sec, dg = secenekKur(k['ar'], celdirici(k['ar'], arHavuz, 8710 + i), 8760 + i)
        sorular.append({"id": kid, "tip": "anlam", "zorluk": zorluk(k['u']),
    ...
    for i in range(10):
        ...
        oid += 1
    #  c) harf sıralama — kısa ve boşluksuz kelimelerden
    kisa = [k for k in karisik(kelimeler, 8005)
    ...
    for i, k in enumerate(kisa[4:8]):
        ...
        for h in karisik(EK_TUS, 8100 + i):
            if len(tus) >= 10:
                break
            if h not in tus:
                tus.append(h)
        sorular.append({"id": oid, "tip": "anlam", "bicim": "yazma", "zorluk": 3,
                        ...
                        "cevapYazi": cevap, "tuslar": karisik(tus, 8200 + i)})
        oid += 1
    ...
    cd = karisik(cumleler, 8006)

    #  a) Bu cümlenin anlamı nedir?  (4 şık, Türkçe)
    for i, c in enumerate(cd):
        if cid >= 20025:
            break
        sec, dg = secenekKur(c['tr'], celdirici(c['tr'], cTr, 8720 + i, 3), 8770 + i)
    ...
    for i, c in enumerate(karisik(cumleler, 8007)):
        if cid >= 20037:
            break
        sec, dg = secenekKur(c['ar'], celdirici(c['ar'], cAr, 8730 + i, 3), 8780 + i)
        sorular.append({"id": cid, "tip": "cumle", "zorluk": zorluk(c['u']),
                        "soru": "«%s» cümlesinin Arapçası hangisi?" % c['tr'],
                        "secenekler": sec, "dogru": dg, "arSecenek": True})
        cid += 1
    #  c) Kelimeleri sırala
    for c in karisik([x for x in cumleler if 3 <= len(x['parca']) <= 5], 8008):
        if cid >= 20049:
            break
    ...
    #     Şıkların uzunluğu eşit olsun (yoksa cevap gözle seçilir) diye hem
    #     boşaltılan parça hem de çeldirici havuzu TEK kelimelik parçalarla
    #     sınırlandırıldı.
    def tekKelime(pz):
    ...
    for i, c in enumerate(karisik([x for x in cumleler
                                   if len(x['parca']) >= 3
                                   and tekKelime(x['parca'][-1])], 8009)):
        if cid >= 20061:
            break
        eksik = c['parca'][-1]
        govde = ' '.join(c['parca'][:-1])
        havuz = [y for y in havuzTumu if y != eksik]
        celd = karisik(havuz, 8740 + i)[:3]
        sec, dg = secenekKur(eksik, celd, 8790 + i)
...
def yaz(sorular):
    ...
    satirlar = ',\n'.join('    ' + json.dumps(s, ensure_ascii=False)
                          for s in sorular)
    blok = ('  /* 8. SINIF — muhadese/veri/8_1_1…8_6_3 ders verisinden ÜRETİLDİ.\n'
    ...
    imza = '{ id: "sinif8", ad: "8. Sınıf"'
    if imza in src:                      # varsa değiştir (betik yinelenebilir)
        b = src.index('  /* 8. SINIF —')
        s9 = src.index('  { id: "sinif9"')
        src = src[:b] + blok + src[s9:]
    else:                                 # yoksa sinif9'un ÖNÜNE ekle
        i = src.index('  { id: "sinif9"')
