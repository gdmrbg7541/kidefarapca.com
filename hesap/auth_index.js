...
function moduDegistir() {
    ...
    var _ng=document.getElementById('name-group'); if(_ng) _ng.style.display = isLoginMode ? "none" : "block";
    var _pg=document.getElementById('phone-group'); if(_pg) _pg.style.display = isLoginMode ? "none" : "block";
...
async function authIslemi() {
    ...
    try {
        if (isLoginMode) {
            ...
            const sPhone = (document.getElementById('phone') ? document.getElementById('phone').value : "").trim();
            if (!sName) { errEl.innerText = "Lütfen isminizi girin."; return; }
...
firebase.auth().onAuthStateChanged(async (user) => {
    ...
    if (user) {
        ...
        var _pb = document.getElementById('header-profile-btn'); if (_pb) _pb.style.display = 'inline-block';
        ...
        var _pb2 = document.getElementById('header-profile-btn'); if (_pb2) _pb2.style.display = 'none';
        var _pm = document.getElementById('profile-modal'); if (_pm) _pm.style.display = 'none';
...
// ===== Profilim / Satın Aldıklarım (index) =====
function openProfileModal() {
    const m = document.getElementById('profile-modal');
    if (m) { m.style.display = 'flex'; renderIndexProfile(); }
}
function closeProfileModal() {
    const m = document.getElementById('profile-modal');
    if (m) m.style.display = 'none';
}
function renderIndexProfile() {
    const user = firebase.auth().currentUser;
    if (!user) return;
    ...
    db.collection('kullanicilar').doc(user.uid).get().then(doc => {
        ...
        setT('profile-name', d.name || '-');
        setT('profile-email', d.email || user.email || '-');
        setT('profile-phone', d.phone || '-');
        const roleMap = { student: 'Öğrenci', teacher: 'Öğretmen', admin: 'Yönetici' };
        setT('profile-role', roleMap[d.role] || 'Öğrenci');
        const listEl = document.getElementById('profile-packages');
        if (!listEl) return;
        const pkgs = d.packages || [];
        if (!pkgs.length) {
            listEl.innerHTML = '<p style="color:#7f8c8d; margin:0;">Henüz bir paket almadınız. <a href="kurslar.html?view=offline-list" style="color:#3498db; font-weight:bold; text-decoration:none;">Paketlere göz at →</a></p>';
            return;
        }
        const off = window.DATA_PAKETLER || [];
        const onl = window.DATA_CANLI_PAKETLER || [];
        const rows = pkgs.map(id => {
            const o = off.find(p => String(p.id) === String(id));
            const n = onl.find(p => String(p.id) === String(id));
            const title = o ? o.title : (n ? n.name : ('Paket #' + id));
            const tag = o ? 'Özel Eğitim' : (n ? 'Canlı Ders' : '');
            const color = o ? '#27ae60' : '#8e44ad';
            return '<li style="display:flex; justify-content:space-between; align-items:center; gap:10px; padding:10px 12px; background:#f8f9fa; border-radius:8px; margin-bottom:8px;"><span style="font-weight:600; color:#2c3e50;">' + title + '</span>' + (tag ? '<span style="font-size:0.72em; background:' + color + '; color:white; padding:3px 8px; border-radius:12px; white-space:nowrap;">' + tag + '</span>' : '') + '</li>';
        }).join('');
        listEl.innerHTML = '<ul style="list-style:none; padding:0; margin:0;">' + rows + '</ul>';
    }).catch(e => { console.error('Profil okunamadı:', e); });
}