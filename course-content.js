...
function loadKazanimData() {
    ...
    if(qArea) {
        if (!stepData || !stepData.question || stepData.question.type === 'none') {
            ...
            let qHtml = `<div style="font-size: 3rem; line-height: 1.4; margin-bottom: 30px; font-weight:bold;">${q.text || ''}</div>`;
            
            if (q.type === 'multiple_choice') {
                let optCount = (q.options || []).length;
                let hasLongOption = (q.options || []).some(opt => opt.split(/\\s+/).filter(w => w.trim().length > 0).length > 5);
                
                let containerStyle = `display:flex; flex-direction:column; gap:15px;`;
                if (!hasLongOption) {
                    if (optCount === 4) {
                        containerStyle = `display:grid; grid-template-columns:1fr 1fr; gap:15px;`;
                    } else if (optCount === 5) {
                        containerStyle = `display:flex; flex-wrap:wrap; justify-content:center; gap:15px;`;
                    }
                }
                
                qHtml += `<div style="${containerStyle}">`;
                (q.options || []).forEach((opt, idx) => {
                    let isCorrect = (q.correctOptionIndex === idx);
                    let idAttr = isCorrect ? `id="correct-option-btn"` : '';
                    let itemStyle = ``;
                    if (!hasLongOption && optCount === 5) {
                        itemStyle = `width: calc(33.333% - 10px); box-sizing: border-box;`;
                    }
                    
                    qHtml += `<button class="mc-option-btn" ${idAttr} style="${itemStyle} text-align:left; padding:20px 25px; border:2px solid #e2e8f0; border-radius:16px; background:white; cursor:pointer; font-family: inherit; font-size:1.8rem; box-shadow: 0 4px 6px -1px rgba(0,0,0,0.05); transition:all 0.3s;" onmouseover="if(!this.classList.contains('selected')){ this.style.background='#f0f9ff'; this.style.borderColor='#7dd3fc'; this.style.transform='translateY(-2px)'; this.style.boxShadow='0 10px 15px -3px rgba(0,0,0,0.1)'; }" onmouseout="if(!this.classList.contains('selected')){ this.style.background='white'; this.style.borderColor='#e2e8f0'; this.style.transform='translateY(0)'; this.style.boxShadow='0 4px 6px -1px rgba(0,0,0,0.05)'; }" onclick="selectOption(this, ${isCorrect})"><span style="font-weight:bold; margin-right:15px; color:#0ea5e9; font-family: sans-serif;">${['A','B','C','D','E'][idx]}</span> ${opt}</button>`;
                });
                qHtml += `</div>`;
            } else if (q.type === 'true_false') {
                qHtml += `
                    <div style="display:flex; gap:20px;">
                        <button class="tf-option-btn" data-base-color="#10b981" ${q.isTrue === true ? 'id="correct-option-btn"' : ''} style="flex:1; padding:25px; background:white; border:2px solid #10b981; color:#10b981; border-radius:16px; font-weight:bold; font-size:1.8rem; font-family: inherit; cursor:pointer; box-shadow: 0 4px 6px -1px rgba(16, 185, 129, 0.1); transition:all 0.3s;" onmouseover="if(!this.classList.contains('selected')){this.style.background='#10b981'; this.style.color='white'; this.style.transform='translateY(-2px)'; this.style.boxShadow='0 10px 15px -3px rgba(16, 185, 129, 0.3)'}" onmouseout="if(!this.classList.contains('selected')){this.style.background='white'; this.style.color='#10b981'; this.style.transform='translateY(0)'; this.style.boxShadow='0 4px 6px -1px rgba(16, 185, 129, 0.1)'}" onclick="selectOption(this, ${q.isTrue === true})">Doğru</button>
                        <button class="tf-option-btn" data-base-color="#ef4444" ${q.isTrue === false ? 'id="correct-option-btn"' : ''} style="flex:1; padding:25px; background:white; border:2px solid #ef4444; color:#ef4444; border-radius:16px; font-weight:bold; font-size:1.8rem; font-family: inherit; cursor:pointer; box-shadow: 0 4px 6px -1px rgba(239, 68, 68, 0.1); transition:all 0.3s;" onmouseover="if(!this.classList.contains('selected')){this.style.background='#ef4444'; this.style.color='white'; this.style.transform='translateY(-2px)'; this.style.boxShadow='0 10px 15px -3px rgba(239, 68, 68, 0.3)'}" onmouseout="if(!this.classList.contains('selected')){this.style.background='white'; this.style.color='#ef4444'; this.style.transform='translateY(0)'; this.style.boxShadow='0 4px 6px -1px rgba(239, 68, 68, 0.1)'}" onclick="selectOption(this, ${q.isTrue === false})">Yanlış</button>
                    </div>
                `;
            } else if (q.type === 'matching') {
                qHtml += `<div style="display:flex; justify-content:space-between; gap:15px;">`;
                let leftHtml = `<div style="flex:1; display:flex; flex-direction:column; gap:8px;">`;
                let rightHtml = `<div style="flex:1; display:flex; flex-direction:column; gap:8px;">`;
                ...
                (q.pairs || []).forEach((p, idx) => {
                    leftHtml += `<div class="match-left-item" data-idx="${idx}" data-original-text="${p.left}" style="padding:12px 15px; background:#f8fafc; border:2px solid #e2e8f0; border-radius:12px; text-align:center; font-size:1.6rem; font-family: inherit; font-weight: 600; color:#334155; min-height:45px; display:flex; align-items:center; justify-content:center; box-shadow: 0 4px 6px -1px rgba(0,0,0,0.05);">${p.left}</div>`;
                });
                rights.forEach((r, idx) => {
                    rightHtml += `<div class="match-right-item" data-idx="${r.correctIdx}" style="padding:12px 15px; background:linear-gradient(135deg, #e0f2fe 0%, #bae6fd 100%); border:2px dashed #38bdf8; border-radius:12px; text-align:center; cursor:grab; font-size:1.6rem; font-family: inherit; font-weight: 600; color:#0284c7; touch-action:none; user-select:none; -webkit-user-select:none; box-shadow: 0 4px 6px -1px rgba(56,189,248,0.2);">${r.text}</div>`;
                });
                leftHtml += `</div>`;
                rightHtml += `</div>`;
                qHtml += leftHtml + rightHtml + `</div>`;
                qHtml += `<div style="margin-top:25px; font-size:1.1rem; color:#0284c7; background:#f0f9ff; padding:15px; border-radius:8px; border:1px solid #bae6fd; text-align:center;">ℹ️ Sağdaki mavi kutuları parmağınızla tutup soldaki eşleşmesinin üzerine bırakınız. Tümünü yerleştirdikten sonra <b>Onayla</b> butonuna basınız.</div>`;
            }
            
            qHtml += `<div style="margin-top: 30px; display:flex; justify-content:flex-end;">
                        <button id="confirm-answer-btn" class="btn btn-success" disabled onclick="confirmAnswer()" style="padding: 15px 45px; font-size: 1.4rem; border-radius: 30px; font-weight:bold; box-shadow: 0 6px 12px rgba(32, 201, 151, 0.3); opacity: 0.5; transition: all 0.3s;">Onayla ✔</button>
                      </div>`;
