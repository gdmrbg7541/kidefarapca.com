...
(function () {
    ...
    /* Bir satırın dört biçimini AYNI ANDA yazmaya başlatır; dönen değer
       tahmini süre (ms) — biçimler birlikte başladığı için satırın süresi
       toplamları değil, EN UZUN biçimin süresidir. Satır bitince (en geç
       biten biçimle) sıra bir sonraki satıra geçer. */
    function satirYaz(satir, benim, bitti) {
        var h = harfler[+satir.getAttribute('data-idx')];
        var veri = h ? HARF_YAZIM[h.h] : null;
        if (!veri) return 0;
        var hucreler = satir.querySelectorAll('.ab-tb-h');
        var toplamSure = 0, zincir = [];
        ...
        [].forEach.call(hucreler, function (hc) {
            ...
            if (sure > toplamSure) toplamSure = sure;   /* aynı anda başlıyorlar: en uzunu */
        });

        /* SIRAYLA + SÜREKLİ DÖNGÜ: biçimler art arda yazılır, satır bitince
           kısa bir bekleyişin ardından baştan başlanır. Sınıfta tahtaya
           bakan çocuk animasyonu kaçırırsa bir sonraki turda yakalar.
           Döngü ancak tablo kapanınca, sekmeden çıkılınca ya da yeni
           kelimeye geçilince durur — o zaman yazSira artar ve aşağıdaki
           kimlik denetimi zinciri keser (bkz. tabloYazDur). */
        /* Satırın dört biçimi AYNI ANDA yazmaya başlar (Geylani); satır,
           EN GEÇ biten biçimle tamamlanmış sayılır ve sıra bir sonraki
           satıra geçer. */
        var kalan = zincir.length;
        if (!kalan) { if (bitti) bitti(); return toplamSure; }
        zincir.forEach(function (a) {
            a.bitince = function () {
                a.bitince = null;
                if (--kalan > 0) return;
            ...
            harfDetay.playForm(a, false, true);
        });
    ...
    function tabloYaz(kap) {
        ...
        /* SATIRLAR DA SIRAYLA (Geylani: "aynı anda birden fazla noktada
           birden fazla doğrultuda yazı yazılmamalı" — bütün ekran için).
           Eskiden satırlar 420 ms arayla başlatılıyor ve her biri kendi
           döngüsünde dönüyordu: üç satır, on iki hücre aynı anda yazıyordu.
           Şimdi bir satır bitmeden öteki başlamıyor; tablo bitince kısa bir
           bekleyişin ardından baştan başlanıyor. */
        (function satirOynat(i) {
            if (benim !== yazSira) return;
            if (kap.classList.contains('ab-tablo-kapali')) return;
            ...
            satirYaz(satirlar[i], benim, function () { satirOynat(i + 1); });
        })(0);
        /* kaba tahmin: satır sayısı × tek satırın süresi */
        var toplam = satirlar.length * 2400;
        return toplam;
    }
