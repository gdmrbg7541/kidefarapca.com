// GÜNCELLENDİ: Hatalı kelimeler düzeltildi/kaldırıldı
        ...
        function setupNewGame() { console.log("setupNewGame:", gameMode); let targetWordCount = (gameMode === '1p') ? 20 : 10; gameSettings.words = targetWordCount; console.log("Initial target words:", targetWordCount); if (gameMode === '1p' && p1TimerEl) { p1TimerEl.classList.remove('hidden'); if(p1TimerValueEl) p1TimerValueEl.textContent = formatTime(gameSettings.timerDuration * targetWordCount); } else if (p1TimerEl) p1TimerEl.classList.add('hidden'); const questions1 = generateMasterQuestionSet(targetWordCount); let currentWordCount = gameSettings.words; console.log("P1 Qs generated:", questions1.length, "Current game words:", currentWordCount); if (gameMode === '1p' && currentWordCount < targetWordCount) { totalTimeLeft = gameSettings.timerDuration * currentWordCount; if(p1TimerValueEl) p1TimerValueEl.textContent = formatTime(totalTimeLeft); console.log("Adjusted 1P timer for reduced word count:", totalTimeLeft); } state = { scores: { '1': 0 }, currentRound: -1, playerQuestions: { '1': questions1 }, answers: { '1': [] }, basZaman: Date.now() }; players['1'].scoreEl.textContent = '0'; players['1'].wordEl.textContent = "..."; if (gameMode === '2p') { state.answers['2'] = []; const questions2 = generateMasterQuestionSet(targetWordCount); currentWordCount = gameSettings.words; console.log("P2 Qs generated:", questions2.length, "Final game words:", currentWordCount); state.playerQuestions['1'] = questions1.slice(0, currentWordCount); state.playerQuestions['2'] = questions2.slice(0, currentWordCount); state.scores['2'] = 0; state.playerResponses = { '1': null, '2': null }; state.responseTimes = { '1': null, '2': null }; players['2'].scoreEl.textContent = '0'; players['2'].wordEl.textContent = "..."; } setupProgressBars(); modal.style.display = 'none'; if (gameSettings.words > 0) { if (gameMode === '1p') startGameTimer(); nextQuestion(); } else { console.error("No questions generated!"); alert("Yeterli kelime bulunamadı, ana menüye dönülüyor."); returnToMenu(); } }
